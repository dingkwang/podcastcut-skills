...
5. 对审查稿任务，优先直接执行 review-canvas skill 自带脚本：
`/Users/lincolnwang/podcastcut-skills/webapp/backend/skills/review_canvas/generate_review_data.py`
6. 应先产出或更新 `transcript.json`，必要时再产出 `corrected.json`，然后在 workspace 根目录生成或更新 `review_data.json`
...
9. 只要 DashScope FunASR 或 OpenRouter Gemini 中至少一个可用，就不要生成空壳 `review_data.json`
10. 文件必须是合法 JSON；如果第一次写坏了，必须继续修到能通过 `/Users/lincolnwang/podcastcut-skills/webapp/backend/.venv/bin/python /Users/lincolnwang/podcastcut-skills/webapp/backend/skills/review_canvas/validate_review_data.py review_data.json`
