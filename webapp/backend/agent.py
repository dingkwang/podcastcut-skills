...
2. 在 workspace 根目录生成或更新 `review_data.json`
3. `review_data.json` 必须严格符合 `review_data.schema.json`
4. 输出的数据模型要与 PodcastCut 审查画布一致：顶层包含 `audio_url`、`audio_duration`、`sentences`、`blocks`、`fineEdits`
5. 文件必须是合法 JSON；如果第一次写坏了，必须继续修到能通过 `python .claude/skills/review_canvas/validate_review_data.py review_data.json`

如果信息不完整，也先生成一个最小可用的 `review_data.json`，不要只口头描述而不落盘。"""
