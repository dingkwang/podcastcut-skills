...
```bash
python .claude/skills/voice_clone/transcribe.py <音频文件> [--speakers N]
```
...
```bash
python .claude/skills/voice_clone/correct.py <transcript.json> --speakers '{"0":"Alice","1":"Bob"}'
```
...
```bash
python .claude/skills/voice_clone/extract.py <音频文件> <transcript.json> <输出目录> [--duration 15]
```
...
```bash
python .claude/skills/voice_clone/create_model.py --name "说话人名" --samples "sample1.wav,sample2.wav"
```
...
```bash
python .claude/skills/voice_clone/tts_batch.py <corrected.json> <voice_models.json> <输出目录>
```
...
单条合成：
```bash
python .claude/skills/voice_clone/tts.py --text "要合成的文字" --model-id <模型ID> --output <输出.mp3>
```

## Step 6: 合并音频

```bash
python .claude/skills/voice_clone/merge.py <片段目录> <输出.mp3> [--pattern "segment_*.mp3"]
```
